...
export function registerPrompts(server, vault) {
  ...
  server.registerPrompt(
    ...
    async ({ note_path_or_title }) => {
      ...
      return userMessage(`Please provide a structured summary of the following note from my Marky vault:
...
4. **Suggested Related Topics / Connections**`);
  ...
  server.registerPrompt(
    "daily_review",
    {
      title: "Review recent daily notes",
      description:
    ...
    async ({ days_back }) => {
      ...
      // Daily notes are YYYY-MM-DD.md at the vault root (and, for older
      // vaults, under Daily/) — not a folder filter.
      const dailyNotes = vault.listDailyNotes({ limit: count });

      const contents = dailyNotes.map(
        (note) => `### ${note.title} (${note.relativePath})\n${note.body}\n`
      );

      return userMessage(`Here are my recent daily notes from my Marky vault:

${contents.join("\n---\n\n") || "No daily notes found in this vault."}
...
4. **Recommended Focus for Today**`);
  ...
  server.registerPrompt(
    ...
    async ({ note_path_or_title }) => {
      ...
      const noteTitles = notes.map((n) => n.title).filter((t) => t !== target.title);

      return userMessage(`I want to connect this note to the rest of my Marky vault knowledge graph.
