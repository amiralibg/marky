...
const TemplateModal = ({ isOpen, onClose, onSelectTemplate, onScheduleTemplate }) => {
  ...
  const handleExportTemplates = async () => {
    ...
    try {
      ...
      if (path) {
        addNotification(`Exported ${customTemplates.length} template${customTemplates.length !== 1 ? 's' : ''}`, 'success');
      }
    } catch (err) {
                      ...
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-text-primary mb-1" title={template.name}>
                              {template.name}
                            </h3>
                            <p
