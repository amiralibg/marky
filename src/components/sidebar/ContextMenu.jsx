...
const ContextMenu = ({
  ...
  onRename,
  onShowTemplate,
  // Non-zero when the right-clicked row is part of a multi-row selection: the
  ...
  const handleAction = async (action) => {
    try {
      if (action === "newNote") {
        ...
        if (selectionCount > 1) {
          // The sidebar owns the multi-row delete, confirmation included.
          onDeleteSelection?.();
          onClose();
          return;
        }
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      if (hasFilePath) {
        addNotification(`${item.type === "note" ? "Note" : "Folder"} deleted`, "success", 5000, {
          label: "Undo",
          callback: async () => {
  ...
  return (
    <>
      ...
      >
        {selectionCount > 1 && (
          <div className="px-3 py-2 text-xs text-text-muted border-b border-glass-border">
            {selectionCount} items selected
          </div>
        )}
        {(item.type === "folder" || item.type === "note") && (
          <>
            {selectionCount > 1 ? null : (
              <>
                <button
                ...
                {item.type === "note" && (
                  ...
                  >
                    <svg
                      className={`w-4 h-4 ${isNotePinned ? "text-amber-400 fill-amber-400" : ""}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
